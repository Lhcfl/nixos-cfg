...
- [`programs/`](programs/) 按程序划分的 HM 配置，由 `home.nix` 自动导入（`mkDirModule` + `mkIndexDirModule "index.nix"`）
...
- `home.nix` 在 `programs` 下同时使用 `mkDirModule`（顶层 `.nix`）与 `mkIndexDirModule "index.nix"`（递归的 `index.nix`，如 [`programs/nushell/`](programs/nushell/)），对 `modules/` 则使用 `mkRecDirModule` 递归导入全部 `.nix`。
