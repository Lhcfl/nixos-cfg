export def main [...extra: string] {
    ...
    let extra_hint = if ($extra | is-empty) {
        ...
        $"额外提示（用户要求，请优先考虑）：($extra | str join ' ')"
    }
...
- 只有当 status/diff 不足以判断改动意图时，才用只读工具查看当前仓库内的文件；
  不要在仓库外搜索，diff 已经足够清楚时不要调用工具'
    ...
    let emit = {|text| if $can_stream { print -n $text } }
            ...
            if $event == null {
                ...
                match $event.type {
                    ...
                    "tool_execution_start" => {
                        let arg = ($event.args.path? | default ($event.args.file_path? | default ($event.args.pattern? | default "")))
                        do $emit $"\n🔧 ($event.toolName) ($arg)\n"
                        null
                    }
                    "tool_execution_end" => {
    ...
    let last_message = (if ($assistant | is-empty) { null } else { $assistant | last })
